...
**Фреймворки**: Vitest (unit/integration) + Playwright (E2E). Точный текущий объём тестов, покрытие и статус подтверждаются CI или завершённым совместимым запуском; локальные Windows bind-mount запуски могут timeout при загрузке модулей.
