...
| Application database | PostgreSQL stores Brain ID, pseudonym, game sessions/results/timing, XP/rating/streak data, and feedback/ideas that can contain free text. | Profile-linked product and potentially sensitive contextual data. | Server-side persistence remains current product behavior. | Still open. The [#221 audit](production-source-of-truth-audit.md) identified `scripts/check-migration-baseline.mjs` as a metadata-only instrument for confirming schema identity, but could not run it: that requires the production `DATABASE_URL`, which is correctly scoped to a reviewer-gated environment. Retention and deletion owner remain undeclared. |
...
| Cookies and third parties | Source search and public response inspection found no observed `Set-Cookie`, Google Analytics, Yandex Metrika, Clarity, or active Sentry SDK integration. | No observed analytics cookie path. | Evidence is scoped to the inspected code/responses. | `Set-Cookie` absence re-confirmed on the reachable public hosts in the [#221 audit](production-source-of-truth-audit.md). Unreachable hosts remain uninspected. |
| Infrastructure | Nginx reverse proxies forward `X-Real-IP` and `X-Forwarded-For`, so client source IP reaches the application process and Nginx access logs. | Technical/log data. | **Retention is undeclared.** No `access_log`, rotation, or retention policy exists anywhere in the repository. | Boundary mapped in the [#221 audit](production-source-of-truth-audit.md). Log fields, access list, retention period and deletion owner remain unverified and must be recorded by an operator with host access. |
...
- [#221](https://github.com/xodapi/kognitika/issues/221), deployment and technical-data-boundary audit. Findings recorded in [`production-source-of-truth-audit.md`](production-source-of-truth-audit.md).
