...
- [~] Add module metadata, lifecycle ownership, and collector integration to each supported cognitive engine. Schulte, N-Back, and Stroop are adopters; remaining engines are pending.
- [~] Record `trial_started`, `trial_answered`, checkpoints, and exactly one terminal event where applicable. Schulte, N-Back, and Stroop record start, answers where available, and completion; checkpoint adoption remains module-specific.
...
- [~] Add deterministic synthetic fixtures for each module family. Schulte, N-Back, and Stroop coverage is added; remaining module families are pending.
