...
export default function InvestorSummaryPage() {
  useEffect(() => {
    ...
    document.querySelector('meta[name="description"]')?.setAttribute('content', 'Короткое описание Kognitika для инвестора: продукт, раунд, ключевые проверки и контакт.');
  }, []);
  ...
  return <div className="investor-summary-page" id="top">
    ...
    <main id="summary-main">
      <section className="summary-hero"><div className="summary-container"><p className="summary-kicker">Коротко для инвестора · ранний раунд</p><h1>Kognitika — регулярная практика внимания, памяти и решений с измеримым прогрессом</h1><p>Когда ИИ берёт на себя рутину, ценность самостоятельного внимания, памяти и решений становится заметнее.</p><div className="summary-actions"><a href="#contact">Запросить материалы <Mail size={17} aria-hidden="true" /></a><a href={PRODUCT_URL} target="_blank" rel="noreferrer">Открыть продукт <ExternalLink size={17} aria-hidden="true" /></a></div></div></section>
      <section className="summary-section summary-container"><div className="summary-grid"><article><p>Что уже есть</p><h2>Работающий продукт без обязательного устройства</h2><ul><li><Check />Когнитивные тренажёры и история занятий</li><li><Check />Прогресс, достижения и продуктовая аналитика</li><li><Check />Веб-версия и мобильные направления</li></ul></article><article><p>Что проверяет раунд</p><h2>Станет ли практика привычкой и спросом</h2><span>Сначала доказываем ценность ежедневного цикла, затем расширяем персональный слой и только после этого проверяем дополнительный контекст от устройств.</span></article></div></section>
      <section className="summary-section summary-dark"><div className="summary-container"><p className="summary-kicker">Инвестиционное предложение</p><h2>Ранние инвестиции: 5–10 млн ₽</h2><p>Конвертируемый займ с капом оценки 100 млн ₽.</p><div className="summary-results"><h3>Результат этого раунда</h3>{milestones.map(([title, text], index) => <article key={title}><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{title}</strong><p>{text}</p></div></article>)}</div></div></section>
      ...
      <section className="summary-contact" id="contact"><div className="summary-container"><p className="summary-kicker">Следующий шаг</p><h2>Материалы выдаются по персональному приглашению</h2><p>Эта страница не собирает имя, контакты, сообщения или другие персональные данные инвесторов. Для разговора или получения приглашения напишите напрямую в Telegram.</p><a href={CONTACT_URL} target="_blank" rel="noreferrer">Написать в Telegram <ArrowRight size={17} aria-hidden="true" /></a></div></section>
    </main>
  </div>;
}
