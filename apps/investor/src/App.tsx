...
const proofItems = [
  ...
  'Прогресс, достижения, баллы опыта и рейтинг',
  'Сохранение результатов на сервере и анализ использования',
...
const horizons = [
  ...
  { number: '02', label: 'После подтверждения', title: 'Сделать путь личным', text: 'Развивать понятные отчёты и рекомендации. ИИ может помогать объяснять результат и выбирать следующий шаг, но не думать вместо человека.', measure: 'Понимание рекомендаций, доверие и повторное использование' },
...
const competitors = [
  { name: 'Muse', entry: 'EEG-повязка', focus: 'Медитация, сон и тренировка психического состояния', position: 'Сенсорное устройство и обратная связь о состоянии', url: 'https://choosemuse.com/' },
  { name: 'Neurable', entry: 'EEG-наушники', focus: 'Паттерны фокуса и подсказки для пауз', position: 'Опыт начинается с устройства для продуктивности', url: 'https://www.neurable.com/products/mw75neuro' },
  { name: 'IDUN', entry: 'Внутриушная EEG-платформа', focus: 'Когнитивная нагрузка и набор средств разработки', position: 'Сенсорная инфраструктура для профессиональных сценариев', url: 'https://iduntechnologies.com/use-case/cognitive-workload' },
  { name: 'Sens.ai', entry: 'Домашняя система', focus: 'Оценка и нейрофидбэк', position: 'Премиальный подход, основанный на устройстве', url: 'https://sens.ai/' },
...
const glossary = [
  ['ИИ / языковая модель', 'ИИ — искусственный интеллект. Языковая модель умеет работать с текстом, искать связи и готовить черновики. На этой странице это будущий инструмент поддержки, а не текущая функция Kognitika.'],
  ...
  ['Для людей / для организаций', 'Продукт для частных пользователей и продукт для организаций — два разных способа выхода на рынок.'],
    ...
    <div className="site-shell" id="top">
      ...
      <main id="main">
        <section className="hero container" aria-labelledby="hero-title"><div className="hero-copy">
          <p className="eyebrow hero-eyebrow"><span className="status-dot" /> Работающий продукт · ранний раунд</p>
          <h1 id="hero-title">Kognitika — регулярная практика<br /><em>внимания, памяти и решений</em><span>с измеримым прогрессом</span></h1>
          <p className="hero-context">Когда ИИ берёт на себя рутину, ценность самостоятельного внимания, памяти и решений становится заметнее.</p>
          <p className="hero-context">Сегодня Kognitika объединяет когнитивную практику и измеримый прогресс внутри платформы. Совместимые устройства — следующий добровольный этап, только после подтверждения пользы базовой практики.</p>
          <div className="hero-actions"><a className="button button-primary" href="#contact">Запросить материалы <Mail size={17} aria-hidden="true" /></a><a className="button button-ghost" href={PRODUCT_URL} target="_blank" rel="noreferrer">Открыть продукт <ExternalLink size={17} aria-hidden="true" /></a><a className="text-link hero-summary-link" href="/summary">Коротко для инвестора <ArrowRight size={16} aria-hidden="true" /></a></div>
          <p className="hero-note">Не медицинский продукт. ИИ, устройства и расширенная персонализация ниже отмечены как направления развития, а не как функции текущей версии.</p>
        ...
        <section className="section differentiation container"><SectionIntro eyebrow="Наша ставка" title="Ежедневная практика без обязательного устройства" text="Kognitika начинает с доступной практики: занятие, результат и следующий шаг. Устройства могут добавить контекст позднее, если это подтвердит пользу для пользователя." /><div className="differentiation-grid"><article><Route aria-hidden="true" /><span>01</span><h3>Практика, а не каталог</h3><p>Занятие, результат и следующий шаг объединены в один повторяемый путь.</p></article><article><Fingerprint aria-hidden="true" /><span>02</span><h3>Данные под контролем</h3><p>Псевдонимная учётная запись, минимизация данных и контролируемый экспорт.</p></article><article><Lightbulb aria-hidden="true" /><span>03</span><h3>ИИ как поддержка</h3><p>Будущая задача ИИ — объяснять и помогать выбирать следующий шаг, а не делать когнитивную работу вместо пользователя.</p></article><article><ShieldCheck aria-hidden="true" /><span>04</span><h3>Расширение после доказательств</h3><p>Носимые устройства и решения для организаций проверяются только после подтверждения пользы и регулярного использования.</p></article></div></section>

        <section className="section market-section" id="market"><div className="container"><SectionIntro eyebrow="Конкурентная карта" title="Рынок подтверждает спрос. Наша точка входа другая" text="Это не рейтинг «кто лучше». Это карта существующих подходов: большинство заметных игроков начинает с сенсора или устройства. Kognitika начинает с ежедневной практики и может подключать дополнительный контекст позднее, только с добровольного согласия пользователя." /><div className="competitor-table" role="region" aria-label="Сравнение подходов на рынке"><div className="competitor-head"><span>Компания</span><span>Точка входа</span><span>Что предлагает</span><span>Подход</span></div>{competitors.map((competitor) => <a className="competitor-row reveal" key={competitor.name} href={competitor.url} target="_blank" rel="noreferrer"><strong>{competitor.name}<ExternalLink size={14} aria-label={`Официальный сайт ${competitor.name}`} /></strong><span>{competitor.entry}</span><span>{competitor.focus}</span><span>{competitor.position}</span></a>)}</div><div className="market-position"><p className="eyebrow">Позиция Kognitika</p><h3>Сначала привычная практика. Затем — только полезный контекст</h3><p>В отличие от моделей, которые начинаются с устройства, Kognitika не требует специального устройства для начала практики. Инвестиции нужны, чтобы доказать ценность цикла и возврат пользователей, улучшить персональную аналитику, а затем проверить один добровольный источник дополнительного контекста.</p></div><p className="source-note">Описания конкурентов основаны на их публичных продуктовых страницах, ссылки ведут на первоисточники. Мы не делаем утверждений об их архитектуре, облачной обработке или регуляторном статусе без отдельной проверки.</p></div></section>

        <section className="section container team-section" id="team"><SectionIntro eyebrow="Команда" title="Основатель отвечает за продукт и инженерную реализацию." /><div className="market-position"><p>Сергей Богорад — основатель, отвечающий за продукт и инженерную реализацию, с 15-летним опытом в крупной компании. Инструменты ИИ ускоряют прототипирование и технические проверки, но ответственность за продукт, архитектуру и решения остаётся у основателя.</p><p>Такой подход помогает проверить продуктовую гипотезу с минимальным бюджетом до найма. Технический сооснователь — следующий плановый найм после подтверждения спроса.</p></div></section>
        ...
        <section className="section container ask-section" id="ask"><div className="proof-panel ask-panel"><div className="proof-copy"><p className="eyebrow">Инвестиционное предложение</p><h2>Ищем ранние инвестиции:<br /><em>5–10 млн ₽</em></h2><p>На условиях конвертируемого займа с капом оценки 100 млн ₽.</p><a className="button button-primary" href="#contact">Запросить материалы <Mail size={17} aria-hidden="true" /></a></div><div className="ask-copy"><p>Средства идут на цели из раздела «Первый фокус»: ежедневный путь пользователя, мобильный опыт, проверка возврата и готовности платить.</p><p>Следующий раунд — после того как эти данные появятся и станут проверяемыми.</p></div></div></section>

        <section className="section evidence-section container"><div className="evidence-heading"><p className="eyebrow">Готовность к инвестициям</p><h2>Какие данные появятся<br /><em>до следующего раунда.</em></h2><p>Публичными станут только проверяемые показатели с источником, датой и владельцем.</p></div><div className="evidence-checklist">{['Активные пользователи и завершённые занятия', 'Возврат на 1 / 7 / 30 день', 'Повторное использование ежедневного пути', 'Переход на платную версию и готовность платить', 'Стоимость привлечения и срок окупаемости', 'Интервью и конкретные примеры ценности'].map((item, index) => <div key={item}><span>{String(index + 1).padStart(2, '0')}</span><p>{item}</p><CircleDashed aria-label="Требует подтверждения" /></div>)}</div></section>
        ...
        <section className="contact-section" id="contact"><div className="contact-glow" /><div className="container contact-content"><p className="eyebrow">Следующий разговор</p><h2>Запросить материалы<br /><em>или назначить разговор</em></h2><p>Оставьте только удобный способ связи. Мы используем заявку, чтобы ответить по вашему запросу, и не добавляем контакты в публичные рассылки.</p><form className="investor-lead-form" onSubmit={submitLead} aria-describedby="lead-privacy-note">
            ...
            <p className="lead-privacy-note" id="lead-privacy-note">Мы используем имя, организацию и контакт только для ответа на запрос. Заявка передаётся в закрытый рабочий чат Telegram команды Kognitika, не добавляется в публичные рассылки; для удаления данных напишите нам в Telegram. Минимальные данные, ограничение частоты заявок и защита от автоматических отправок. Отвечаем в течение одного рабочего дня.</p>
