    ...
    <div className="site-shell" id="top">
      ...
      <main id="main">
        <section className="hero container" aria-labelledby="hero-title"><div className="hero-copy">
          ...
          <p className="hero-lead">Kognitika развивает работающий продукт с тренажёрами и историей прогресса в понятный цикл: короткое занятие, измеримый результат, следующий шаг и возвращение к практике.</p>
          <p className="hero-context">AI берёт на себя всё больше интеллектуальной рутины. Поэтому особенно важны навыки, которые остаются за человеком: удерживать внимание, учиться, проверять ответ и принимать решение. Мы помогаем их регулярно практиковать, а не обещаем заменить мышление AI.</p>
        ...
        <section className="section container problem" id="product"><SectionIntro eyebrow="Почему сейчас" title="Когда AI снимает рутину, самостоятельное мышление становится заметнее" text="AI может подготовить черновик, найти информацию или структурировать текст. Но человеку всё ещё нужно поставить задачу, удержать контекст, заметить ошибку, проверить вывод и принять решение. Это не медицинское утверждение, а продуктовая гипотеза Kognitika." /><div className="problem-grid"><article className="feature-card dark-card reveal delay-1"><Brain size={26} aria-hidden="true" /><h3>Практика навыков</h3><p>Короткие упражнения на внимание, память и скорость реакции дают человеку конкретное действие, а не абстрактный совет «думать лучше».</p></article><article className="feature-card reveal delay-2"><Route size={26} aria-hidden="true" /><h3>Понятный путь</h3><p>После занятия важно увидеть свой результат и понять, что делать дальше. Так отдельные упражнения могут превратиться в привычку.</p></article><article className="feature-card accent-card reveal delay-3"><TimerReset size={26} aria-hidden="true" /><h3>Проверяемая ставка</h3><p>Сейчас задача команды — измерить, увеличивает ли этот путь возвращение к практике и готовность пользователя платить.</p></article></div></section>
        ...
        <section className="section container ask-section" id="ask"><div className="proof-panel ask-panel"><div className="proof-copy"><p className="eyebrow">Инвестиционное предложение</p><h2>Ищем pre-seed:<br /><em>5–10 млн ₽</em></h2><p>На условиях конвертируемого займа с капом оценки 100 млн ₽.</p><a className="button button-primary" href={`mailto:${CONTACT_EMAIL}`}>Написать команде <Mail size={17} aria-hidden="true" /></a></div><div className="ask-copy"><p>Средства идут на цели из раздела «Первый фокус»: ежедневный путь пользователя, мобильный опыт, проверка возврата и готовности платить.</p><p>Следующий раунд — после того как эти данные появятся и станут проверяемыми.</p></div></div></section>
        ...
        <section className="contact-section" id="contact"><div className="contact-glow" /><div className="container contact-content"><p className="eyebrow">Следующий разговор</p><h2>Обсудим продукт,<br /><em>доказательства и рост</em></h2><p>Открыты к разговору о продуктовой стратегии, доказательной базе и следующем этапе Kognitika.</p><div className="hero-actions"><a className="button button-primary" href={`mailto:${CONTACT_EMAIL}`}>Написать команде <Mail size={17} aria-hidden="true" /></a><a className="button button-ghost light-ghost" href={PRODUCT_URL} target="_blank" rel="noreferrer">Открыть Kognitika <ExternalLink size={17} aria-hidden="true" /></a></div></div></section>
      </main>
